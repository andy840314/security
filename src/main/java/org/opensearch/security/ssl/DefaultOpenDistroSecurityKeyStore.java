...
public class DefaultOpenDistroSecurityKeyStore implements OpenDistroSecurityKeyStore {
    ...
    public DefaultOpenDistroSecurityKeyStore(final Settings settings, final Path configPath) {
        ...
        if(!OpenDistroSecuritySSLPlugin.OPENSSL_SUPPORTED && OpenSsl.isAvailable() && (settings.getAsBoolean(SSLConfigConstants.SECURITY_SSL_HTTP_ENABLE_OPENSSL_IF_AVAILABLE, true) || settings.getAsBoolean(SSLConfigConstants.SECURITY_SSL_TRANSPORT_ENABLE_OPENSSL_IF_AVAILABLE, true) )) {
