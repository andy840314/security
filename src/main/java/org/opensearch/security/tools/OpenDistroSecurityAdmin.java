...
public class OpenDistroSecurityAdmin {
    ...
    public static int execute(final String[] args) throws Exception {
        ...
        final Settings.Builder settingsBuilder = Settings
                ...
                .put(SSLConfigConstants.SECURITY_SSL_TRANSPORT_ENABLE_OPENSSL_IF_AVAILABLE, OpenDistroSecuritySSLPlugin.OPENSSL_SUPPORTED && useOpenSSLIfAvailable)
    ...
    private static int issueWarnings(Client tc) {
        ...
        if(nir.getNodes().size() > 0) {
            ...
            String securityVersion = pluginInfos.stream().filter(p->p.getClassname().equals("org.opensearch.security.OpenDistroSecurityPlugin")).map(p->p.getVersion()).findFirst().orElse("<unknown>");
